...
book_csv = pd.read_csv("dataset/books_info.csv")
data = loader.load()
...
If you don't know the answer, just say that you don't know the answer or that you don't make up an answer. Don't same book twice.
{context}
...
def search_books_by_kwords(genre: str, keyword: str, top_n: int = 3):
    matches = book_csv[book_csv['Book Name'].str.contains(keyword, case=False, na=False)]

    if genre is not None:
        matches = matches[matches['Genre'].str.contains(genre, case=False, na=False)]
    
    return matches['Book Name'].tolist()[:top_n]
